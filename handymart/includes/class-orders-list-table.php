...
class WPOrders_List_Table extends WP_List_Table {
    ...
    function get_columns() {
        return $columns = array(
            ...
            'col_orders_id' => __('ID'),
            'col_orders_customer_name' => __('Khách hàng'),
            'col_orders_amount' => __('Tổng tiền'),
            'col_orders_date' => __('Ngày'),
            'col_orders_options' => __('Tùy chọn')
        );
    }
                ...
                foreach ($columns as $column_name => $column_display_name) {
                    ...
                    switch ($column_name) {
                        ...
                        case "col_orders_options":
                            echo '<td ' . $attributes . '><a href="' . $printLink . '" target="_blank">In hóa đơn</a> | ';
    ...
    function get_bulk_actions() {
        ...
        if ($status == APPROVED) {
            $actions = array(
                'unapprove' => __('Không duyệt', 'nvt_orders'),
                'cancel' => __('Hủy', 'nvt_orders')
            );
        } elseif ($status == CANCELLED) {
            $actions = array(
                'approve' => __('Duyệt', 'nvt_orders'),
                'restore' => __('Phục hồi', 'nvt_orders'),
                'delete' => __('Xóa vĩnh viễn', 'nvt_orders')
            );
        } else {
            $actions = array(
                'approve' => __('Duyệt', 'nvt_orders'),
